...
load1=`uptime | sed 's/,//g' | awk '{ print $10 }' | cut -d'.' -f1`
load2=`uptime | sed 's/,//g' | awk '{ print $11 }' | cut -d'.' -f1`

if [ $load1 -ge 16 ] || [ $load2 -ge 16 ]; then
   load1=`uptime | sed 's/,//g' | awk '{ print $10 }'`
   load2=`uptime | sed 's/,//g' | awk '{ print $11 }'`
